...
import static io.tourniquet.proxy.HttpProxyVerticle.TransmissionDirection.RCV;
import static io.tourniquet.proxy.HttpProxyVerticle.TransmissionDirection.SND;
...
public class HttpProxyVerticle extends AbstractVerticle {
    ...
    /**
     * Global tracker for request IDs. Even if multiple proxy verticles are deployed, the request number is globally unique (inside a jvm)
     */
    ...
    private void processRequest(final HttpServerRequest req) {
        ...
        if (req.method() == HttpMethod.CONNECT) {
            ...
            final HttpClientRequest outgoingRequest = this.httpClient.request(req.method(), info.port, info.host, req.uri(), targetResponse -> {
                ...
                if (LOG.isTraceEnabled()) {
                    LOG.trace("[{}] Received response {} {},\n-HEADER-\n{}\n-HEADER-",
                              ...
                              toString(targetResponse.headers()));
                }

                final ChunkDataHandler responseHandler = new ChunkDataHandler(conId, RCV, req.response());
                req.response().setChunked(true);
                req.response().setStatusCode(targetResponse.statusCode());
                req.response().headers().setAll(targetResponse.headers());
                targetResponse.handler(responseHandler);
                targetResponse.endHandler((v) -> responseHandler.endTransmission());
            }).setChunked(true);
    ...
    /**
     * Helper class to parse hostname and port from a string hostname:port with the option to define a default port during parsing
     */
    ...
    private class ChunkDataHandler implements Handler<Buffer> {
        ...
        private final TransmissionDirection tmDir;
        private final WriteStream<Buffer> out;
        private int dataInFlight = 0;
        private boolean indicateEnd;
        private Runnable onCloseHandler;

        public ChunkDataHandler(final int conId, final TransmissionDirection tmDir, final WriteStream<Buffer> out) {
        ...
        public void handle(final Buffer data) {
            ...
            stats.receivedBytes(tmDir, data.length());

            if (LOG.isTraceEnabled()) {
                LOG.trace("[{}] {}: {} bytes\n---\n{}\n---", conId, tmDir, data.length(), data.toString(Charset.defaultCharset()));
            } else if (LOG.isDebugEnabled()) {
                LOG.debug("[{}] {}: {} bytes", conId, tmDir, data.length());
            }

            dataInFlight++;

            Vertx.currentContext().owner().eventBus().send(tmDir.getAddress(), data, transmitData(data));
        }

        /**
         * Indicate the transmission is complete. The method finishes the writing to the stream, if there is no data in processing that needs to be completed.
         */
            ...
            return reply -> {
                if (reply.succeeded()) {
                    ...
                    if (modifiedData.length() > 0) {
                        ...
                        stats.transmitBytes(tmDir, modifiedData.length());
                        out.write(modifiedData);
                    } else {
                        stats.dropBytes(tmDir, originalData.length());
                        LOG.debug("[{}] writing no data", conId);
                    }
                } else {
                    LOG.debug("[{}] Dataprocessing failed", conId, reply.cause());
                    stats.transmitBytes(tmDir, originalData.length());
