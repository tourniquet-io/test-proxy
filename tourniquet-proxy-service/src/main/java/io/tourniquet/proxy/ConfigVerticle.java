...
/**
 * Configuration handler for the proxy.
 * The following endpoints can be used to configure the proxy:
 *
 * <ul>
 *     <li><code>/config</code>
 *          <ul>
 *              <li>GET - retrieves the current handler dispatcher configuration</li>
 *              <li>PUT - enables a specifc data handler. Must be a valid handler from the list (see GET), use empty string for disable proxy for that direction<pre>
 *                  { "incoming" : "handlerName"}
  *              </pre></li>
  *          </ul>
 *     </li>
 *     <li><code>/config/ttr</code>
 *          <ul>
 *              <li>POST - sets a timer for resetting the proxy (disabling handler for direction)
 *              <pre>{
...
public class ConfigVerticle extends AbstractVerticle {
    ...
    public void start() throws Exception {
        ...
        router.route("/eb/*").handler(SockJSHandler.create(vertx).bridge(new BridgeOptions().addOutboundPermitted(new PermittedOptions())));
        router.route().handler(StaticHandler.create());

        this.server = vertx.createHttpServer(opts).requestHandler(router::accept).listen();
    }
    ...
    private void putConfig(final RoutingContext ctx) {
        ...
        String handler = config.getString(dir); //empty string to disable
